...
public class CustomBullets : MonoBehaviour
{
    ...
    public void Explode()
    {
        ...
        //Check for player
        if(Physics.Raycast(transform.position, transform.forward, out rayHit, explosionRange, whatIsPlayer))
        {
            if(rayHit.collider.gameObject.GetComponent<PlayerMovement>())
                rayHit.collider.GetComponent<PlayerMovement>().TakeDamage(explosionDamage);
            ...
            if (objectToTp != null && tpOnExplosion) 
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (isVanished && !canStillExplodeWhenVanished) 
            return;
        ...
        if (explodeOnTouch && collision.collider.CompareTag("Player"))
            Explode();
